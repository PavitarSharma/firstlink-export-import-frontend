...
import {
  ...
  hairSizes,
  shoesSizes,
} from "../../components/admin/add-products/data";
...
import UploadImages, {
  UploadImageRef,
} from "../../components/admin/add-products/UploadImages";
...
import MultiSelect from "../../components/layout/inputs/MultiSelect";
import { IProduct } from "../../types";
import { axiosPrivate } from "../../config/api";

type Schema = z.infer<typeof EditProductSchema>;
...
const navigate = useNavigate()
  ...
  useEffect(() => {
    if (product) {
      ...
      setValue("shippingPrice", `${product.shippingPrice}`);
      setValue("discountPrice", `${product.discountPrice}`);
      setValue("sku", product.sku);
      setValue("barcode", product.barcode);
      setValue("quality", product.quality);
      const sizes = product.sizes.map((size: string) => ({
        value: size,
        label: size,
      }));
      uploadImageRef.current?.setDefaultMedia(product.images);
  ...
  const onSubmit = async (values: Schema) => {
    ...
    try {
    await editProduct(responseData, id as string);
    ...
    <div className="flex items-center justify-center flex-col gap-6 mb-10">
      <div className="w-full">
        <div className="flex items-center justify-between w-full">
          ...
          <div>
            <Button label="Edit Product" onClick={handleSubmit(onSubmit)} />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-6">
            <div>
              <div className="shadow bg-white rounded-md p-4 w-full">
                ...
                <UploadImages hasEdit ref={uploadImageRef} />
              </div>
            </div>
